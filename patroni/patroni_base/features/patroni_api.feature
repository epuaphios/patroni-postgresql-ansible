...
Scenario: check API requests on a stand-alone server
	...
	When I issue a POST request to http://127.0.0.1:8009/switchover with {"leader": "postgres0"}
	...
	When I issue an empty POST request to http://127.0.0.1:8009/failover
	Then I receive a response code 400
	When I issue a POST request to http://127.0.0.1:8009/failover with {"foo": "bar"}
	Then I receive a response code 400
...
Scenario: check dynamic configuration change via DCS
	...
	And Response on GET http://127.0.0.1:8009/patroni contains pending_restart after 11 seconds
	When I issue a GET request to http://127.0.0.1:8009/config
	Then I receive a response code 200
...
Scenario: check the scheduled restart
	Given I issue a PATCH request to http://127.0.0.1:8009/config with {"postgresql": {"parameters": {"superuser_reserved_connections": "6"}}}
	Then I receive a response code 200
		And Response on GET http://127.0.0.1:8009/patroni contains pending_restart after 5 seconds
	Given I issue a scheduled restart at http://127.0.0.1:8009 in 3 seconds with {"role": "replica"}
	Then I receive a response code 202
		And I sleep for 4 seconds
		And Response on GET http://127.0.0.1:8009/patroni contains pending_restart after 10 seconds
	Given I issue a scheduled restart at http://127.0.0.1:8009 in 3 seconds with {"restart_pending": "True"}
	Then I receive a response code 202
		And Response on GET http://127.0.0.1:8009/patroni does not contain pending_restart after 10 seconds
...
Scenario: check the switchover via the API in the pause mode
	Given I issue a POST request to http://127.0.0.1:8009/switchover with {"leader": "postgres0", "candidate": "postgres1"}
	Then I receive a response code 200
