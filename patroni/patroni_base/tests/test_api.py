...
class MockRestApiServer(RestApiServer):
    ...
    def __init__(self, Handler, request, config=None):
        ...
        config = config or {'listen': '127.0.0.1:8009', 'auth': 'test:test', 'certfile': 'dumb', 'verify_client': 'a'}
...
class TestRestApiServer(unittest.TestCase):
    ...
    @patch.object(BaseHTTPServer.HTTPServer, '__init__', Mock())
    def setUp(self):
        self.srv = MockRestApiServer(Mock(), '', {'listen': '*:8009', 'certfile': 'a', 'verify_client': 'required'})
    ...
    def test_reload_config(self):
        ...
        with patch.object(socket.socket, 'setsockopt', Mock(side_effect=socket.error)):
            self.srv.reload_config({'listen': ':8009'})
    ...
    def test_socket_error(self):
        self.assertRaises(socket.error, MockRestApiServer, Mock(), '', {'listen': '*:8009'})
